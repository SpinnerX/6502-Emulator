 ...
 * 
 * MnemonicTraits.h
 * - Since opcodes are referred to an abbreviation called Mnemonics
 * - Which means to represent different operations.
 * 
 * New NOTES: (Reference to the notes in Operation.h for more detail as well)
 * - Should do before I forget: take out OperationTypes
 * - Consider that OperationTraits is not something known to the CPU
 * - Simply the CPU only knows what opcodes what to run with each opcode selected.
 * - Simply when we call opcodes they are mnemonic.
 * - Mnemonic simply means calling different opcodes that represent different operations for users
 *  to easily recall these opcodes for different operations that they do.
 * NOTE: Not instructions, instructions (as in registers) refer to A, X, Y, etc.
 * 
 * 
*/
